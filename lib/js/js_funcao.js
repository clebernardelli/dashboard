...
/* Pra quando for utilizar JQueryUI
$(function() {
    $( ".janela" ).dialog();
});
*/
...
function ajaxSendData(JSONdata) {
        
    var jqxhr = $.ajax({
            data : JSONdata
        })
        .done(function(data){
                   var location = JSONdata.split('=');
                   renderDashboard(location[1], data);            
              })
        .fail(function(){
                   alert("FALHA"); 
              })
    ...
    //Procedimentos para quando a requisição foi realizada mas ainda não obteve resposta

}

function renderDashboard(location, HTMLData) {
    $( "#main" ).html(HTMLData);
}
